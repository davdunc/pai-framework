...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Updating world threat models. This will take several minutes as I research current state for each time horizon.", "voice_id": "${ELEVENLABS_VOICE_ID}"}'
```
...
curl -s -X POST http://localhost:8888/notify \
  -H "Content-Type: application/json" \
  -d '{"message": "World models updated. N horizons refreshed with current research.", "voice_id": "${ELEVENLABS_VOICE_ID}"}'
```
