...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "Testing your idea against all eleven world threat models at TIER tier", "voice_id": "${ELEVENLABS_VOICE_ID}"}'
```
...
curl -s -X POST http://localhost:8888/notify \
  -H "Content-Type: application/json" \
  -d '{"message": "Analysis complete. SUMMARY_OF_EXECUTIVE_VERDICT", "voice_id": "${ELEVENLABS_VOICE_ID}"}'
```
