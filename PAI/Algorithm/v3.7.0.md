...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "MESSAGE", "voice_id": "${ELEVENLABS_VOICE_ID}", "voice_enabled": true}'
```
...
**Voice (FIRST action after loading this file):** `curl -s -X POST http://localhost:8888/notify -H "Content-Type: application/json" -d '{"message": "Entering the Algorithm", "voice_id": "${ELEVENLABS_VOICE_ID}", "voice_enabled": true}'`
...
- PREREQUISITE CHECK (MANDATORY — verify before committing to any approach):

Before selecting effort level or generating ISC, validate that the work CAN be done:

1. **Service/API availability:** Are required APIs enabled, subscribed, and authenticated? (e.g., DAS CMD API enabled on account, Notion connected, API keys present)
2. **Environment constraints:** Does the execution environment support the approach? (e.g., WSL2 → Windows networking, localhost accessibility, port availability)
3. **Tool/dependency viability:** Are critical dependencies reliable? (Flag packages with <1000 downloads or <6 months age for risk assessment)
4. **Domain-specific fast path:** Is there a known optimal approach for this domain? Check CONTEXT_ROUTING.md. (e.g., Notion → loadPageChunk API, not browser scraping)

If ANY prerequisite fails or is uncertain, surface it immediately via AskUserQuestion BEFORE proceeding. Do NOT commit to a build plan with unverified prerequisites — this is the #1 cause of wasted Algorithm runs.

OUTPUT:

🔑 PREREQUISITES:
 🔑 [Service/API status — verified or needs confirmation]
 🔑 [Environment constraints — any blockers identified]
 🔑 [Dependency risk — any low-adoption packages flagged]
 🔑 [Domain fast path — optimal approach identified or N/A]
